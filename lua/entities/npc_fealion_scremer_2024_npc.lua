...
util.PrecacheSound("fealion_2024_npc/jump.mp3")
util.PrecacheSound("fealion_scremer_2024_npc/scream.mp3")
util.PrecacheSound("fealion_2024_npc/spring.mp3")
util.PrecacheSound("fealion_2024_npc/aaaa.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local npc_fealion_scremer_2024_npc_acquire_distance = CreateConVar("npc_fealion_scremer_2024_npc_acquire_distance", 2500, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                "The maximum distance at which fealion_scremer_2024 will chase a target.")

local npc_fealion_scremer_2024_npc_spawn_protect = CreateConVar("npc_fealion_scremer_2024_npc_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, fealion_scremer_2024 will not target players or hide within 200 units of a spawn point.")

local npc_fealion_scremer_2024_npc_attack_distance = CreateConVar("npc_fealion_scremer_2024_npc_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of fealion_scremer_2024's attack.")

local npc_fealion_scremer_2024_npc_attack_interval = CreateConVar("npc_fealion_scremer_2024_npc_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between fealion_scremer_2024_npc's attacks.")

local npc_fealion_scremer_2024_npc_attack_force = CreateConVar("npc_fealion_scremer_2024_npc_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of fealion_scremer_2024's attack. Higher values throw things farther.")

local npc_fealion_scremer_2024_npc_smash_props = CreateConVar("npc_fealion_scremer_2024_npc_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, fealion_scremer_2024 will punch through any props placed in their way.")

local npc_fealion_scremer_2024_npc_hiding_scan_interval = CreateConVar("npc_fealion_scremer_2024_npc_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "fealion_scremer_2024 will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant fealion_scremer_2024s are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")



local npc_fealion_scremer_2024_npc_hiding_repath_interval = CreateConVar("npc_fealion_scremer_2024_npc_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to fealion_scremer_2024's hiding spot will be redetermined every X seconds.")

local npc_fealion_scremer_2024_npc_chase_repath_interval = CreateConVar("npc_fealion_scremer_2024_npc_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of fealion_scremer_2024's target will be redetermined every X seconds.")

local npc_fealion_scremer_2024_npc_expensive_scan_interval = CreateConVar("npc_fealion_scremer_2024_npc_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local npc_fealion_scremer_2024_npc_force_download = CreateConVar("npc_fealion_scremer_2024_npc_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download fealion_scremer_2024 resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear fealion_scremer_2024!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 0.5
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (npc_fealion_scremer_2024_npc_force_download:GetBool()) then
	resource.AddWorkshop("2845656134")
end

util.AddNetworkString("fealion_scremer_2024_npc_nag")
util.AddNetworkString("fealion_scremer_2024_npc_nagresponse")
util.AddNetworkString("fealion_scremer_2024_npc_navgen")
...
local VECTOR_fealion_scremer_2024_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_fealion_scremer_2024_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_fealion_scremer_2024_npc: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other fealion_scremer_2024s, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC() and
	        ent:Health() > 0 and
	        class ~= "npc_fealion_scremer_2024_npc" and
	        not class:find("bullseye"))
end

--HACK!!! Because this is an issue a lot of people keep asking me about.
hook.Add("PlayerSpawnedNPC", "fealion_scremer_2024MissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_fealion_scremer_2024_npc") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.fealion_scremer_2024_npc_HasBeenNagged) then return end
	ply.fealion_scremer_2024_npc_HasBeenNagged = true

	-- Try to explain why fealion_scremer_2024_npc isn't working.
	net.Start("fealion_scremer_2024_npc_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("npc_fealion_scremer_2024_npc: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_fealion_scremer_2024_npc: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("fealion_scremer_2024_npc_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply._npc_HasBeenNagged = false
		return
	end
	...
	if (not IsValid(spawnPoint)) then
		net.Start("fealion_scremer_2024_npc_navgen")
			net.WriteBit(false)
		net.Send(ply)
	...
	for _, fealion_scremer_2024_npc in pairs(ents.FindByClass("npc_fealion_scremer_2024_npc")) do
		fealion_scremer_2024_npc:Remove()
	end

	print("npc_fealion_scremer_2024_npc: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "fealion_scremer_2024NavGen", navEndGenerate)
	else
		print("npc_fealion_scremer_2024_npc: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_fealion_scremer_2024_npc_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `npc_fealion_scremer_2024_npc_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (npc_fealion_scremer_2024_npc_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_fealion_scremer_2024_npc_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. fealion_scremer_2024 means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * npc_fealion_scremer_2024_npc_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * npc_fealion_scremer_2024_npc_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not npc_fealion_scremer_2024_npc_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_fealion_scremer_2024_npc_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_fealion_scremer_2024_npc_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "fealion_2024_npc/spring.mp3" or "fealion_2024_npc/jump.mp3"), 350, 100)
	end
end
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > npc_fealion_scremer_2024_npc_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > npc_fealion_scremer_2024_npc_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(npc_fealion_scremer_2024_npc_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > npc_fealion_scremer_2024_npc_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= npc_fealion_scremer_2024_npc_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= npc_fealion_scremer_2024_npc_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= npc_fealion_scremer_2024_npc_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= npc_fealion_scremer_2024_npc_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local fealion_scremer_2024_npcsLastFrame = 0
hook.Add("Tick", "fealion_scremer_2024Benchmark", function()
	local now = SysTime()
	local numfealion_scremer_2024s = #ents.FindByClass("npc_fealion_scremer_2024_npc")

	if (timeAll ~= 0) then

		local fealion_scremer_2024_npcTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    fealion_scremer_2024_npcTime / fealion_scremer_2024_npcsLastFrame, fealion_scremer_2024_npcTime, frameTime, (fealion_scremer_2024_npcTime / frameTime) * 100, fealion_scremer_2024_npcsLastFrame, numThink, (numThink / fealion_scremer_2024_npcsLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	fealion_scremer_2024_npcsLastFrame = numfealion_scremer_2024s
end)]]
...
killicon.Add("npc_fealion_scremer_2024_npc", "fealion_scremer_2024_npc/npc_fealion_scremer_2024_npc_killicon", color_white)
language.Add("npc_fealion_scremer_2024_npc", "fealion_scremer_2024")
...
local function DevPrint(devLevel, msg)
	if (developer:GetInt() >= devLevel) then
		print("npc_fealion_scremer_2024_npc: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a fealion_scremer_2024.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_fealion_scremer_2024_npc_music_volume = CreateConVar("npc_fealion_scremer_2024_npc_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by fealion_scremer_2024. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another fealion_scremer_2024_npc comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, fealion_scremer_2024_npcs do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_fealion_scremer_2024_PANIC_COUNT fealion_scremer_2024_npcs are this close, or an equivalent score.
local MUSIC_fealion_scremer_2024_PANIC_COUNT = 8 -- That's a lot of fealion_scremer_2024_npc.

local MUSIC_fealion_scremer_2024_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_fealion_scremer_2024_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("npc_fealion_scremer_2024_npc") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("fealion_scremer_2024PanicMusicUpdate")
	...
	if (panicMusic == nil) then
		if (IsValid(LocalPlayer())) then
			panicMusic = CreateSound(LocalPlayer(), "fealion_scremer_2024_npc/scream.mp3")
			panicMusic:Stop()
		else
	...
	if (npc_fealion_scremer_2024_npc_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		panicMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "npc_fealion_scremer_2024_npc") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_fealion_scremer_2024_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear fealion_scremer_2024 taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(npc_fealion_scremer_2024_npc_music_volume:GetFloat(), 0, 1))
...
local function startTimer()
	if (not timer.Exists("fealion_scremer_2024PanicMusicUpdate")) then
		timer.Create("fealion_scremer_2024PanicMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local fealion_scremer_2024_npcMaterial = Material("fealion_scremer_2024_npc/npc_fealion_scremer_2024_npc.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(fealion_scremer_2024_npcMaterial)
...
hook.Add("OnEntityCreated", "fealion_scremer_2024Initialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_fealion_scremer_2024_npc") then return end

	local fealion_scremer_2024_npcEntTable = scripted_ents.GetStored("npc_fealion_scremer_2024_npc")

	table.Merge(ent, fealion_scremer_2024_npcEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("fealion_scremer_2024_npc_removed", fealion_scremer_2024_npcDeregister)
end)

hook.Add("NetworkEntityCreated", "fealion_scremer_2024NetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_fealion_scremer_2024_npc") then return end
...
surface.CreateFont("fealion_scremer_2024HUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("fealion_scremer_2024HUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("fealion_scremer_2024 is studying this map.", "fealion_scremer_2024HUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "fealion_scremer_2024HUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "fealion_scremer_2024HUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "fealion_scremer_2024HUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "fealion_scremer_2024HUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("fealion_scremer_2024_npc_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "fealion_scremer_2024NavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. fealion_scremer_2024 doesn't even know where to start with this map.
...
local function navGenerate()
	net.Start("fealion_scremer_2024_npc_nagresponse")
		net.WriteBit(true)
	net.SendToServer()
end
...
local function navWarning()
	Derma_Query([[It will take a while for fealion_scremer_2024 to figure this map out.
...
-- Lazy
net.Receive("fealion_scremer_2024_npc_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! fealion_scremer_2024 doesn't know this map.
Would you like fealion_scremer_2024 to learn it?]],
		            "This map is currently not fealion_scremer_2024-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! fealion_scremer_2024 doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to fealion_scremer_2024.]],
		            "This map is currently not fealion_scremer_2024-compatible!",
		            "Ok", nagAgain,
...
--
list.Set("NPC", "npc_fealion_scremer_2024_npc", {
	Name = "fealion scremer 2024",
	Class = "npc_fealion_scremer_2024_npc",
